...
* phylo0 The phylogeny to be plotted. This should be time-calibrated (e.g. using strap:datePhylo())
* occ The dataset to be used for plotting. This has to either be a list() containing data.frames to be used as an argument for the function divdistr_() (or any other function it is overridden with) as parameter "table", or a data.frame() or matrix() containing an x collumn and collumns matching the names of the phylogenetic tree’s tip.labels.
* ages Optional data matrix used for cropping each spindle to the known stratigraphic range of the taxon. If provided, this should take the form of a matrix with row names being the same as the trees tip.labels, and two collumns named "FAD" and "LAD" giving minimum and maximum geological ages for each taxon.
* dscale Vertical scaling parameter for the spindles. May need manual adjustment, depending on the desired scale.
* txt.x Either a single number of a vector of the same length as the number of tree tips giving the horizontal position for labels (if labels==TRUE). In this case, the second collumn of ages is used, placing the labels at the end of the spindles, but the first and last values are replaced by 66 in order to place labels within the plot boundaries
...
As an example, the object diversity_table contains a time series of by-stage diversity estimates for the same taxa made using the divDyn packages (in this case divRT):
